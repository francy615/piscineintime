...
function renderMieiOrdini() {
  ...
  list.innerHTML = miei.map(o => {
    ...
    const _stepLabels = ['Ricevuto','In lavorazione','Completato'];
    const stepsBar = stato !== 'annullato'
      ? '<div class="storico-progress">' +
          _stepLabels.map((s, i) => {
            let cls = (i <= step) ? ' sp-done' : '';
            if (i === step) cls += ' sp-current';
            const dot = '<div class="sp-step' + cls + '"><div class="sp-dot"></div><div class="sp-label">' + s + '</div></div>';
            const line = i < _stepLabels.length - 1 ? '<div class="sp-line' + (i < step ? ' sp-done' : '') + '"></div>' : '';
            return dot + line;
          }).join('') + '</div>'
...
let _moFiltroAttivo = 'tutti';

function renderMieiOrdiniPage() {
  const user = getCurrentUser();
  const list = document.getElementById('moList');
  const heroTitle = document.getElementById('moHeroTitle');
  const heroSub   = document.getElementById('moHeroSub');
  const avatar    = document.getElementById('moAvatar');
  const filtri    = document.getElementById('moFiltri');
  ...
  if (!user) {
    ...
    list.innerHTML = `
      <div class="mo-empty-state mo-login-gate">
        <div class="mo-empty-icon">🔐</div>
        <h3>Accedi per vedere i tuoi ordini</h3>
        <p>Devi essere loggato per visualizzare lo storico dei tuoi ordini.</p>
        <button class="btn-submit" onclick="openUserLogin()">👤 Accedi</button>
      </div>`;
    return;
  }
  ...
  if (heroTitle) heroTitle.textContent = 'Ciao, ' + user.nome + '!';

  const tutti  = (window.data && window.data.ordini) ? window.data.ordini : [];
  const miei   = tutti.filter(o => o.email && o.email.toLowerCase() === user.email.toLowerCase())
                       .slice().reverse(); // più recenti prima

  if (heroSub) {
    heroSub.textContent = miei.length === 0
      ? 'Non hai ancora effettuato nessun ordine'
      : miei.length + (miei.length === 1 ? ' ordine effettuato' : ' ordini effettuati');
  }

  // Populate stats strip
  const statsEl = document.getElementById('moStats');
  if (statsEl) {
    statsEl.style.display = miei.length > 0 ? 'grid' : 'none';
    const sNuovi = miei.filter(o => (o.stato||'nuovo') === 'nuovo').length;
    const sLav   = miei.filter(o => (o.stato||'') === 'in_lavorazione').length;
    const sComp  = miei.filter(o => (o.stato||'') === 'completato').length;
    document.getElementById('moStatTutti').textContent = miei.length;
    document.getElementById('moStatNuovi').textContent = sNuovi;
    document.getElementById('moStatLav').textContent   = sLav;
    document.getElementById('moStatComp').textContent  = sComp;
  }
  ...
  if (miei.length === 0) {
    list.innerHTML = `
      <div class="mo-empty-state">
        ...
        <p>Non hai ancora effettuato nessun ordine. Scopri i nostri prodotti!</p>
        <button class="btn-primary" onclick="showPage('home')">🏠 Vai ai prodotti</button>
      </div>`;
    return;
  }

  _moFiltroAttivo = _moFiltroAttivo || 'tutti';
  _renderMoList(miei, _moFiltroAttivo);
}

function _renderMoList(miei, filtro) {
  const list = document.getElementById('moList');
  if (!list) return;

  const filtered = filtro === 'tutti' ? miei : miei.filter(o => (o.stato || 'nuovo') === filtro);

  if (filtered.length === 0) {
    list.innerHTML = `<div class="mo-empty-state mo-empty-filtro">
      <div class="mo-empty-icon">🔍</div>
      <h3>Nessun ordine in questa categoria</h3>
      <p>Prova a selezionare un altro filtro.</p>
    </div>`;
    return;
  }

  const statoMeta = {
    nuovo:          { label:'Nuovo',          icon:'🟢', cls:'stato-nuovo',          color:'#065f46', bg:'#d1fae5' },
    in_lavorazione: { label:'In lavorazione', icon:'🟡', cls:'stato-in_lavorazione', color:'#92400e', bg:'#fef3c7' },
    completato:     { label:'Completato',     icon:'🔵', cls:'stato-completato',     color:'#1e40af', bg:'#dbeafe' },
    annullato:      { label:'Annullato',      icon:'🔴', cls:'stato-annullato',      color:'#991b1b', bg:'#fee2e2' }
  };

  const pagLabel = { ritiro:'Ritiro in negozio', bonifico:'Bonifico bancario', assegno:'Contrassegno' };

  list.innerHTML = filtered.map(o => {
    const stato = o.stato || 'nuovo';
    const meta  = statoMeta[stato] || statoMeta.nuovo;

    // Prodotti
    const prods = Array.isArray(o.prodotti) && o.prodotti.length
      ? o.prodotti.map(p => `<span class="storico-prodotto-tag">${p.prodotto} <strong>× ${p.qty}</strong></span>`).join('')
      : `<span class="storico-prodotto-tag">${o.prodotto || '–'}</span>`;
    ...
    const step = stato === 'annullato' ? -1
               : stato === 'nuovo'     ?  0
               : stato === 'in_lavorazione' ? 1 : 2;
    const stepLabels = ['Ricevuto','In lavorazione','Completato'];
    const progressBar = stato !== 'annullato'
      ? `<div class="storico-progress">${
          stepLabels.map((s, i) => {
            let cls = i <= step ? ' sp-done' : '';
            if (i === step) cls += ' sp-current';
            const line = i < stepLabels.length - 1
              ? `<div class="sp-line${i < step ? ' sp-done' : ''}"></div>` : '';
            return `<div class="sp-step${cls}"><div class="sp-dot"></div><div class="sp-label">${s}</div></div>${line}`;
          }).join('')
        }</div>`
      : `<div class="storico-annullato-bar">Ordine annullato</div>`;

    // Totale
    const totale = Array.isArray(o.prodotti) && o.prodotti.length
      ? o.prodotti.reduce((sum, p) => sum + (parseFloat(p.prezzo) || 0) * (parseInt(p.qty) || 1), 0)
      : 0;
    const totaleHtml = totale > 0
      ? `<span class="storico-foot-item">💰 Totale: <strong>€ ${totale.toFixed(2).replace('.',',')}</strong></span>`
      : '';

    return `<div class="storico-card mo-card">
      <div class="storico-card-head">
        <div class="storico-card-id">
          <span class="storico-id-label">Ordine</span>
          <span class="storico-id-val">${o.id}</span>
      ...
      <div class="storico-prodotti">
        <div class="storico-section-label">Prodotti ordinati</div>
        <div class="storico-prodotti-list">${prods}</div>
      </div>
      <div class="storico-card-foot">
        <span class="storico-foot-item">🏦 <strong>${pagLabel[o.pagamento] || o.pagamento || '–'}</strong></span>
        ${o.indirizzo ? `<span class="storico-foot-item">📍 ${o.indirizzo}${o.citta ? ', ' + o.citta : ''}</span>` : ''}
        ${totaleHtml}
        ${o.note ? `<span class="storico-foot-item">📝 ${o.note}</span>` : ''}
      </div>
    </div>`;
  }).join('');
}

function moFiltro(btn, stato) {
  _moFiltroAttivo = stato;
  document.querySelectorAll('.mo-filtro').forEach(b => b.classList.remove('active'));
