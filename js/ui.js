...
function makeProductCard(p, catKey) {
  ...
  const prezzoHtml = (p.prezzo !== undefined && p.prezzo !== null && p.prezzo !== '')
    ? `<div class="product-prezzo">€ ${Number(p.prezzo).toFixed(2).replace('.', ',')}</div>`
    : '';
  const cartBtn = catKey && ORDERABLE_CATS.has(catKey)
    ...
    : '';
  return `<div class="product-card"><div class="product-img">${imgHtml}</div><div class="product-body"><h4>${p.nome}</h4><p>${p.desc}</p>${prezzoHtml}${cartBtn}</div></div>`;
}
